...
CURLcode Curl_disconnect(struct connectdata *conn, bool dead_connection)
{
  ...
  {
    /* If a connection is closed by the server, we do not find out until bytes
     * are written to it. So we can create an NTLM header (and set start to
     * NTLMSTATE_TYPE1_SENT) and when we try to write the request, find out the
     * connection is dead. When this happens we choose a new connection and
     * retry the request, so we can't clear the request's state yet because we
     * need to use it to generate the new request. If we clear the state, the
     * new request will not get an NTLM header. An unfortunately common case
     * is where a server always sends "Proxy-Authenticate: NTLM" and then
     * closes the connection - it wants the first request on every connection
     * to have an NTLM type-1 header. If we don't put an NTLM header on the
     * retry, we will never output any NTLM headers because the first
     * connection we try to respond on is always closed.
     *
     * After NTLMSTATE_TYPE2_RECEIVED, this connection is in the middle of a
     * handshake (or if the state is NTLMSTATE_AUTHORIZED, it's already
     * authenticated) and in this case if it is closed NTLM has to be restarted
     * from scratch.
     *
     * In other words, if the state is:
     *   NTLMSTATE_NONE: nothing to do
     *   NTLMSTATE_PICKED: do not clear anything, need picked to stay as NTLM
     *                     when retrying request
     *   NTLMSTATE_TYPE1_SENT: ditto
     *   NTLMSTATE_TYPE2_RECEIVED: clear request's NTLM status because we need
     *                             to start over completely on a new conn
     *   NTLMSTATE_TYPE3_SENT: ditto
     *   NTLMSTATE_AUTHORIZED: ditto
     *
     * Note: this applies only to the NTLM state held in the session handle.
     * The NTLM state in the connection itself is always reset, since the
     * connection is going away and if it is reused it will be for a fresh
     * connection. It is only the request's NTLM state that must sometimes be
     * saved for a new connection.
     */
    int has_host_ntlm = conn->ntlm.state > NTLMSTATE_TYPE1_SENT &&
                        conn->ntlm.state <= NTLMSTATE_AUTHORIZED;
    int has_proxy_ntlm = conn->proxyntlm.state > NTLMSTATE_TYPE1_SENT &&
...
static bool safe_strequal(const char* a, const char* b)
{
  if(a && b)
    return strequal(a, b);
  if(!a && !b)
...
/*
 * Given one filled in connection struct (named needle), and another existing
 * connection (named check), this function will detect if all the significant
 * details are exactly the same and thus check should be used instead.
 *
 ...
 */
static bool
ConnectionMatches(struct SessionHandle *data,
                  struct connectdata *needle,
                  struct connectdata *check,
                  bool canPipeline,
                  size_t pipeLen,
                  size_t max_pipe_len,
                  bool inNTLMHandshake,
                  bool inProxyNTLMHandshake)
{
  DEBUGASSERT(check);
  ...
  /* Do not stomp on any connection which is doing an NTLM handshake */
  if(!inNTLMHandshake && check->ntlm.state >= NTLMSTATE_PICKED &&
     check->ntlm.state < NTLMSTATE_AUTHORIZED)
    return FALSE;
  if(!inProxyNTLMHandshake && check->proxyntlm.state >= NTLMSTATE_PICKED &&
     check->proxyntlm.state < NTLMSTATE_AUTHORIZED)
    return FALSE;

  /* If NTLM is in use, make sure the credentials match. (These tests must
   * use safe_strequal because we have observed crashes when user or passwd
   * is NULL. */
  if(data->state.authhost.picked == CURLAUTH_NTLM ||
     data->state.authhost.picked == CURLAUTH_NTLM_WB) {
    if(!safe_strequal(needle->user, check->user) ||
       !safe_strequal(needle->passwd, check->passwd))
      return FALSE;
  }
  if(data->state.authproxy.picked == CURLAUTH_NTLM ||
     data->state.authproxy.picked == CURLAUTH_NTLM_WB) {
    if(!safe_strequal(needle->proxyuser, check->proxyuser) ||
       !safe_strequal(needle->proxypasswd, check->proxypasswd))
      return FALSE;
  }
  ...
  if(!needle->bits.httpproxy || needle->handler->flags&PROTOPT_SSL ||
    ...
    if(needle->handler->protocol & CURLPROTO_FTP) {
      /* This is FTP, verify that we're using the same name and password as
       * well (NTLM credentials were checked above) */
      if(!safe_strequal(needle->user, check->user) ||
         !safe_strequal(needle->passwd, check->passwd)) {
    ...
    if(!(check->bits.proxy &&
         ...
         Curl_raw_equal(needle->proxy.name, check->proxy.name) &&
         needle->port == check->port)) {
      /* This is not the same proxy connection */
      return FALSE;
    }
  }
  ...
  if(canPipeline) {
    ...
    /* We can't use the connection if the pipe is penalized (which is
     * impossible if pipeLen is 0) */
    if(pipeLen > 0 && Curl_pipeline_penalized(data, check))
      return FALSE;
  }
...
{
  struct connectdata *check;
  struct connectdata *lastconn = NULL;
  struct connectbundle *bundle;
  ...
  *force_reuse = FALSE;
  ...
  /* If we are in the middle of NTLM negotiation, we MUST continue using the
   * same connection. Note that we choose the same connection even for
   * NTLMSTATE_PICKED, since we want to respond on the connection which the
   * server requested NTLM for. */
  Curl_getconnectinfo(data, &lastconn);
  if(lastconn) {
    bool inNTLMHandshake = lastconn->ntlm.state >= NTLMSTATE_PICKED &&
    ...
    if(inNTLMHandshake || inProxyNTLMHandshake) {
      ...
      if(ConnectionMatches(data, needle, lastconn,
                           canPipeline, pipeLen, max_pipe_len,
                           inNTLMHandshake, inProxyNTLMHandshake)) {
        infof(data, "Reusing previous connection #%ld for NTLM\n",
              lastconn->connection_id);
        *usethis = lastconn;
        *force_reuse = TRUE;
        return TRUE;
      }
      infof(data, "Could not continue NTLM auth on connection #%ld\n",
            lastconn->connection_id);
      return FALSE;
    }
  }
  ...
  if(bundle) {
    ...
    while(curr) {
      ...
      if(ConnectionMatches(data, needle, check, canPipeline, pipeLen,
                           max_pipe_len, FALSE, FALSE)) {
        if(canPipeline) {
...
#define LEAST_PATH_ALLOC 256
  ...
  */
  ...
  if(reuse && !force_reuse && IsPipeliningPossible(data, conn_temp)) {
    ...
    if(pipelen > 0) {
      infof(data, "Found connection %d, but the pipe is too long (%d)\n",
      ...
      if(conn_temp->bundle->num_connections < max_host_connections &&
        ...
        infof(data, "We can reuse, but we create a new connection anyway\n");
      }
    }
  }
...
static CURLcode do_init(struct connectdata *conn)
{
  ...
  /* Make sure the NTLM state of the connection matches the request that has
   * just been assigned to it. */
  Curl_http_ntlm_checkstate(conn, /*proxy*/ FALSE);
  Curl_http_ntlm_checkstate(conn, /*proxy*/ TRUE);
